...
def bookstore():
    ...
    if 'keyword' in request.args and 'page' in request.args:
        ...
        cursor.execute('SELECT * FROM PRODUCT WHERE Prd_NAME LIKE %s', ('%' + search + '%',))
...
def cart():
    ...
    if request.method == 'POST':
        if "pid" in request.form:
            ...
            Order_List.add_order({'mid': current_user.id, 'order_date': time, 'total': total, 'format': format, 'tno': tno})
